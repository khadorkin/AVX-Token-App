...
const config = {};
const dispatch = (msg, value) => {
  // eslint-disable-next-line no-console
  console.warn(msg, value);
};
const dispatcher = msg => ev => {
  // eslint-disable-next-line no-console
  console.warn(msg, ev);
};

const AudioTag = props => <audio {...props} />; // eslint-disable-line jsx-a11y/media-has-caption
const VideoTag = props => <video {...props} />; // eslint-disable-line jsx-a11y/media-has-caption
...
export default class Player extends React.Component {
  state = {
    ...
    devices: {},
    mediaRef: component => {
      const elem = this.onRef(component);
      this.state.elem = elem;
      if (elem) {
        console.log('Videostream.init');
        videostream(this.props.getPlayingTorrentSummary().mediaFile, elem);
      }
    },
  };
  ...
  render() {
    ...
    state.getPlayingTorrentSummary = this.props.getPlayingTorrentSummary;
    state.getPlayingTorrentProgress = this.props.getPlayingTorrentProgress;
    // onMouseMove={dispatcher('mediaMouseMoved')}
    return (
      <div className="player" onWheel={handleVolumeWheel}>
...
function renderMedia(state) {
  ...
  if (mediaElement) {
    ...
    // Save video position
    const file = state.getPlayingTorrentSummary();
  ...
  // Show the media.
  return (
    <div key="letterbox" className="letterbox" onMouseMove={dispatcher('mediaMouseMoved')}>
      {mediaTag}
      {renderOverlay(state)}
    </div>
  );
...
function renderAudioMetadata(state) {
  const fileSummary = state.getPlayingTorrentSummary();
...
function renderLoadingSpinner(state) {
  ...
  const isProbablyStalled =
    state.playing.isStalled || new Date().getTime() - state.playing.lastTimeUpdate > 2000;
  if (!isProbablyStalled) return;

  const fileProgress = state.getPlayingTorrentProgress() || {};
...
function renderPlayerControls(state) {
  ...
  const captionsClass =
    !state.playing.subtitles || state.playing.subtitles.tracks.length === 0
      ? 'disabled'
      : state.playing.subtitles.selectedIndex >= 0
        ? 'active'
        : '';
  const prevClass = false ? '' : 'disabled';
  const nextClass = false ? '' : 'disabled';

  const elements = [
    <div key="playback-bar" className="playback-bar">
      {renderLoadingBar(state)}
      <div key="cursor" className="playback-cursor" style={playbackCursorStyle} />
      <div
        key="scrub-bar"
        className="scrub-bar"
        draggable="true"
    ...
    <i key="play" className="icon play-pause float-left" onClick={dispatcher('playPause')}>
  ...
  const isCastingAnywhere = castTypes.some(castType => state.playing.location.startsWith(castType));
  ...
  const volumeIcon = `volume_${
    volume === 0 ? 'off' : volume < 0.3 ? 'mute' : volume < 0.6 ? 'down' : 'up'
  }`;
  const volumeStyle = {
    background:
      `${'-webkit-gradient(linear, left top, right top, ' + 'color-stop('}${volume *
        100}%, #eee), ` + `color-stop(${volume * 100}%, #727272))`,
  };
...
function renderLoadingBar(state) {
  ...
  const torrentSummary = state.getPlayingTorrentProgress() || {};
  if (!torrentSummary.progress) {
    return null;
  }

  // Find all contiguous parts of the torrent which are loaded
  const fileInfo = state.getPlayingTorrentSummary();

  if (!torrentSummary.progress || !fileInfo.mediaFile) return null;
  const torrent = fileInfo.mediaFile._torrent;
  const path = fileInfo.mediaFile.path;
  const fileProg = torrent.files.filter(f => f.path === path)[0];

  if (!fileProg) return null;

  const parts = [];
  let lastPiecePresent = false;
  for (let i = fileProg._startPiece; i <= fileProg._endPiece; i++) {
    const partPresent = Bitfield.prototype.get.call(torrent.bitfield, i);
    if (partPresent && !lastPiecePresent) {
      parts.push({ start: i - fileProg._startPiece, count: 1 });
    } else if (partPresent) {
  ...
  const loadingBarElems = parts.map((part, i) => {
    const style = {
      left: `${100 * part.start / (fileProg._endPiece - fileProg._startPiece)}%`,
      width: `${100 * part.count / (fileProg._endPiece - fileProg._startPiece)}%`,
    };
